 ...
 * a key named `firebase.webapikey` that contains the Firebase project Web API Key.
 */
@Named
@ViewScoped
public class FirebaseAuthSignIn implements Serializable {
    ...
    public String submit() {
        ...
        JsonObject credentials = Json.createObjectBuilder()
                ...
                .build();
        try {
            FirebaseAuthSignInResponsePayload loginFirebaseAuthSignInResponsePayload = _loginService.signIn(firebaserestapiKey, credentials);
            _firebaseAuthSignInSession.setFirebaseAuthSignInResponsePayload(loginFirebaseAuthSignInResponsePayload);
            _firebaseAuthSignInSession.setUsername(username);
